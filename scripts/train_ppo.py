...
import bobcat_env
from lib import model, test_net, calc_logprob, make_parser, parse_args, make_env, make_nets
import logging
import numpy as np
import torch
...
def calc_adv_ref(trajectory, net_crt, states_v, device="cpu"):
    ...
    """
    # we ask the critic to convert states into values
    ...
    for val, next_val, (exp,) in zip(reversed(values[:-1]),
        ...
        if exp.done:  # no prior reward to take into account
            delta = exp.reward - val
            last_gae = delta
        else:  # the current step is not terminal
...
if __name__ == "__main__":
    rospy.init_node('movingbobcat_gym_train_ppo', anonymous=True, log_level=rospy.WARN)
    rate = rospy.Rate(120)
    parser = make_parser()
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        for step_idx, exp in enumerate(exp_source):
            ...
            traj_adv_v, traj_ref_v = calc_adv_ref(  # takes trajectory with steps and calculates advantages for the actor and reference values for the critic training
                trajectory, net_crt, traj_states_v, device=device)
            mu_v = net_act(traj_states_v)
            old_logprob_v = calc_logprob(  # calculate the logarithm of probability of the actions taken
            ...
            # For every batch, we extract the portions from the corresponding arrays and do the critic and the actor
            # training separately.
            for epoch in range(PPO_EPOCHS):
                ...
                for batch_ofs in range(0, len(trajectory),
                    ...
                    loss_value_v = F.mse_loss(value_v.squeeze(-1), batch_ref_v)  # calculate the mean squared error loss
                    # with the reference values calculated beforehand.
                    loss_value_v.backward()
                    opt_crt.step()

                    # actor training
                    opt_act.zero_grad()  # set the gradients to zero before starting to do backpropragation
                    ...
                    loss_policy_v = -torch.min(   # minimize the negated clipped objective
