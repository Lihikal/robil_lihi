...
class Controller:
    ...
    soil_type = {0: "dry_sand", 1: "wet_sand"} #, 2:"garbel"}
    # soil = "wet_sand"
 ...
 ## definitions
    ...
    def __init__(self):
        ...
        self.pub4 = rospy.Publisher('/soil_type_backprop', String, queue_size=10)
        ...
        while self.flag == 0:
            ...
            if self.bobby_true_vel.x < 0.01 and self.depth > 0.05 and self.loader_pos.position.x > -1.25:
                 ...
                 results = check_soil(inputs, targets, self.bobcat_vel, self.depth, ((self.depth * H * 1.66) / 2))
                 print(results)
                 # str = str(results[0][0])
                 if str(results[0][0])=='True':
                     self.pub4.publish(self.soil_type[0])
                 if str(results[0][1])=='True':
                     self.pub4.publish(self.soil_type[1])
                     print('in')
                 # print(self.soil_type[1])

                 # create(self.bobcat_vel, self.depth, ((self.depth * H *1.66) /2))
                 # exec(open('user_window.py').read())
